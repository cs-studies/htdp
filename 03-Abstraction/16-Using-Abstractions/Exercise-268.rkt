...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Exercise-268) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;
;; Exercise 268.
...
(define (sort-by-diff l)
  (local (
          ;; IR IR -> Boolean
          ;; Compares inventory records
          ;; by the difference between the two prices.
          (define (cmp-prices-diff ir1 ir2)
            (< (diff-prices ir1) (diff-prices ir2)))

          ;; IR -> Number
          ;; Calculates the difference between
          ;; the two prices of the given ir.
          (define (diff-prices ir)
            (- (ir-sell ir) (ir-bought ir))))
    (sort l cmp-prices-diff)))
