...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Exercise-524) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;
;; Exercise 524.
;; Exploit the accumulator-oriented data representation to modify solve.
;; The revised function produces the list of states
;; that lead from the initial PuzzleState to the final one.
;; Also consider creating a movie from this list,
;; using render-mc to generate the images. Use run-movie to display the movie.
...
(define (create-next-states l0)
  (local
    ((define all-parents
       (local (;; [List-of PuzzleState] -> [List-of Parent]
                         ...
                         (local
                           ((define (parents/one/a parents acc)
                              (cond
                                ...
                                [else
                                 (local
                                   ((define p (ps->p (first parents)))
                                    (define newacc (if (member? p acc) acc (cons p acc))))
                                   (parents/one/a (rest parents) newacc))])))
                           (parents/a (rest states)
                                      (parents/one/a (ps-parents (first states)) a)))])))
...
;; Produces the list of states
;; that lead from the initial PuzzleState to the final one.
(define (solve state0)
  (local ((define (final? ps)
            (local ((define left (ps-left ps)))
              (and (zero? (side-mis left)) (zero? (side-can left)))))

          (define (solve* los)
            (local ((define found (filter final? los)))
              (cond
                [(cons? found)
                 (cons (ps->p (first found)) (ps-parents (first found)))]
...
(define CAN (overlay (circle SIZE "solid" "orange")
                     (circle (+ 1 SIZE) 'solid 'transparent)))
...
(run-movie 3 (map render-mc (reverse (solve ps-1))))
